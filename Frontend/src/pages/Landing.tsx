...
const leftRef = useRef(null)
  ...
  useEffect(() => {
    const revealTimer = setTimeout(()=>setRevealing(true),2000)
...
//animation
  useEffect(() => {
    if(isLoading) return //is black screen is loading do not run the animation
...
const navigate = useNavigate()
  return (
     <>
       {isLoading && (
           <div className={`fixed inset-0 z-[100] bg-black flex justify-center items-center transition-transform duration-1000 ease-in-out
           ${revealing ? "-translate-y-full": "translate-y-0"}`}>
             <div className={"flex gap-6 ml-16 items-center text-center justify-center"}>

               <span className={"font-bold z-[100] text-6xl  font-black-ops text-white"}>K i w i  </span>
               <span className={"font-bold z-[100] text-6xl  font-black-ops text-white"}>E n t e r p r i s e </span>
               <span></span>
             </div>
             </div>
       ...
       <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
         ...
         <section className="pt-32 pb-20 px-4">
           <div className="max-w-7xl mt-12 mx-auto text-center">
            <div className={"w-full flex mx-auto items-center text-center justify-between"}>
              <span
                style={{opacity:0}}
                  className="font-chango text-5xl md:text-7xl font-extrabold tracking-tight mb-6 bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-900 bg-clip-text text-transparent"
              ref={leftRef}>
                Premium Paint
              </span>
